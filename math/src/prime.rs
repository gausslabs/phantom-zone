...
pub(crate) fn two_adic_generator(q: u64, two_adicity: usize) -> u64 {
    assert_eq!((q - 1) % (1 << two_adicity) as u64, 0);
    mod_pow(multiplicative_generator(q), (q - 1) >> two_adicity, q)
}

pub(crate) fn mod_pow(b: u64, e: u64, q: u64) -> u64 {
    BigUint::from(b)
...
pub(crate) fn mod_inv(v: u64, q: u64) -> u64 {
    mod_pow(v, q - 2, q)
}

pub(crate) fn mod_powers(b: u64, q: u64) -> impl Iterator<Item = u64> {
