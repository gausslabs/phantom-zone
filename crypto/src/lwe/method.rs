...
pub fn ksk_gen<R: RingOps + ElemFrom<T>, T: Copy>(
    ring: &R,
    mut ksk: LweKeySwitchKeyMutView<R::Elem>,
    ...
    izip_eq!(
        &ksk.ct_iter_mut().chunks(decomposer.level()),
        sk_from.as_ref(),
    )
    .for_each(|(ksk_i, sk_from_i)| {
        izip_eq!(ksk_i, decomposer.gadget_iter()).for_each(|(ksk_i_j, beta_j)| {
            let pt = LwePlaintext(ring.mul_elem_from(&ring.neg(&beta_j), sk_from_i));
            sk_encrypt(ring, ksk_i_j, sk_to, pt, noise_distribution, &mut rng)
        })
    });
}
...
pub fn key_switch<R: RingOps>(
    ...
    let decomposer = R::Decomposer::new(ring.modulus(), ksk.decomposition_param());
    izip_eq!(&ksk.ct_iter().chunks(decomposer.level()), ct_from.a()).for_each(|(ksk_i, a_i)| {
        izip_eq!(ksk_i, decomposer.decompose_iter(a_i)).for_each(|(ksk_i_j, a_i_j)| {
            ring.slice_scalar_fma(ct_to.as_mut(), ksk_i_j.as_ref(), &a_i_j)
        })
    });
